...
def conllize(delim, fields, files, cols = [], withs = []):
    ...
    with contextlib.ExitStack() as stack:
        files = [stack.enter_context(codecs.open(fname, 'r', encoding='utf-8')) for fname in files]
...
def bios(conll_files, bios_files, fields):
    ...
    with contextlib.ExitStack() as stack:
        conll_files = [stack.enter_context(codecs.open(fname, 'r', encoding='utf-8')) for fname in conll_files]
        bios_files = [stack.enter_context(codecs.open(fname, 'r', encoding='utf-8')) for fname in bios_files]
...
def merger(conll_path, predicted_path, cplaceholder, cdelim, fdelim, ccolnum, fcolnum):
    predictions = []
    with codecs.open(predicted_path, 'r', encoding='utf-8') as fstream:
        for line in fstream:
            line = line.strip()
            items = line.split(fdelim)
            predictions.append(items)

    with codecs.open(conll_path, 'r', encoding='utf-8') as cstream:
        anno_set = 0
        for line in cstream:
            line = line.strip()
            if line == '':
