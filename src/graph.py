...
class DependencyGraph:
	def __init__(self, nodes, edges):
		...
		#print('DependencyGraph',nodes,edges)
		self.G = nx.DiGraph()
		for node_id in nodes:
			self.G.add_node(node_id, word=nodes[node_id][0], pos=nodes[node_id][1])
		self.G.add_node(0, word="ROOT", pos='')
		for edge in edges:
			label = edge[2]
		...
		self.roles = None
		self.sent = " ".join(nodes[nid][0] for nid in sorted(list(nodes.keys())))
		self.gid = None
		self.sid = None
	...
	def pretty(self):
		...
		for n in self.G.nodes():
			if self.G.node[n] != {}:
				...
				pos = self.G.node[n]["pos"]
				head = list(self.G.predecessors(n))[0] if len(list(self.G.predecessors(n))) > 0 else "_"
				dep_label = self.G[head][n]["label"] if len(list(self.G.predecessors(n))) > 0 else "_"
				role = self.G.node[n].get("role", "_")
				pred = self.G.node[n].get("frame", "_")
				s += "\t".join([x for x in [str(gid), str(n), word, pos, str(head), dep_label, role, pred]])+"\n"
		return s
	...
	def find_parent_verb(self, node):
		""" Find the verb dominating the node """
		n = node
		while ((n != 0) and ('VB' not in self.G.node[n]["pos"])):
			n = list(self.G.predecessors(n))[0]
		if (n == 0):
			return -1
		else:
			return n
	...
	def get_head(self, nodes):
		...
		if len(nodes) == 1:
			...
			for node_id in nodes:
				parent = list(self.G.predecessors(node_id))[0]
