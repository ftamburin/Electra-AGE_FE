...
def collect_srl_data(in_fes):  # load SRL data (~frame.elements). All the offsets are shifted by 1!
	...
	for line in in_fes:
		...
		sid = int(line[7])
		role_info = line[8:]
		role_info = [x.upper().replace(' ','_') for x in role_info]
		...
		for x in range(0, len(role_info), 2):
			role_dict[role_info[x]] = fix_tid(role_info[x+1], ":")  # role offsets are given as start:end
		fee_info += [role_dict]
...
def collect_sentence_data(in_sentences):  # load parse data (~all.lemma.tags)
	...
	for line in in_sentences:
		...
		if line:
			...
			num_tok = int(line[0])
			line = line[1:]
			data = [line[x*num_tok:x*num_tok+num_tok] for x in range(0, int(len(line)/num_tok))]  # TODO list comprehension ninja required here
	...
	for sid in sentences:
		if sid in srl_data:
			...
			if sentence is not None:
				nodes = {tid: [sentence[tid]["form"],sentence[tid]["pos"]] for tid in sentence}
...
def get_graphs(src_sentences, src_fes, verbose=False):  # files in, graphs out
	...
	with codecs.open(src_sentences, "r", "utf-8") as in_sentences:
		with codecs.open(src_fes, "r", "utf-8") as in_fes:
			...
			print(src_sentences.split("/")[-1], src_fes.split("/")[-1], "labeled:", len(srl_data),
"parsed:", len(sentences), "graphs:", len(graphs))
			for graph in graphs:
				graph.gid = i
				i += 1
			return graphs
